...
-- @
method :: Monad actM => Method -> ApiaryT exts prms actM m a -> ApiaryT exts prms actM m a
...
stdMethod :: Monad actM => Method -> ApiaryT exts prms actM m a -> ApiaryT exts prms actM m a
stdMethod = method

-- | filter by ssl accessed. since 0.1.0.0.
ssl :: Monad actM => ApiaryT exts prms actM m a -> ApiaryT exts prms actM m a
ssl = function_ (DocPrecondition "SSL required") isSecure

-- | http version filter. since 0.5.0.0.
httpVersion :: Monad actM => HT.HttpVersion -> Html -> ApiaryT exts prms actM m b -> ApiaryT exts prms actM m b
httpVersion v h = function_ (DocPrecondition h) $ (v ==) . Wai.httpVersion

-- | http/0.9 only accepted fiter. since 0.5.0.0.
http09 :: Monad actM => ApiaryT exts prms actM m b -> ApiaryT exts prms actM m b
http09 = Control.Monad.Apiary.Filter.httpVersion HT.http09 "HTTP/0.9 only"

-- | http/1.0 only accepted fiter. since 0.5.0.0.
http10 :: Monad actM => ApiaryT exts prms actM m b -> ApiaryT exts prms actM m b
http10 = Control.Monad.Apiary.Filter.httpVersion HT.http10 "HTTP/1.0 only"

-- | http/1.1 only accepted fiter. since 0.5.0.0.
http11 :: Monad actM => ApiaryT exts prms actM m b -> ApiaryT exts prms actM m b
...
root :: (Functor m, Monad m, Monad actM) => ApiaryT exts prms actM m b -> ApiaryT exts prms actM m b
root = focus' DocRoot Nothing (RootPath:) return

-- | match all subsequent path. since 0.15.0.
anyPath :: (Functor m, Monad m, Monad actM) => ApiaryT exts prms actM m b -> ApiaryT exts prms actM m b
...
-- @
query :: forall exts prms actM a w m b. 
      (ReqParam a, Strategy.Strategy w, MonadIO actM)
      => QueryKey
      -> w a
      -> ApiaryT exts (Strategy.SNext w prms a) actM m b
      -> ApiaryT exts prms actM m b
query QueryKey{..} p =
    focus doc $ \l -> do
        r     <- getRequest
        (q,f) <- getRequestBody
...
-- @
(=?:) :: (MonadIO actM, ReqParam p) => QueryKey -> proxy p
      -> ApiaryT exts (Maybe p ': prms) actM m a -> ApiaryT exts prms actM m a
...
-- @
--
hasQuery :: MonadIO actM => QueryKey -> ApiaryT exts prms actM m a -> ApiaryT exts prms actM m a
hasQuery q = query q (Strategy.Check :: Strategy.Check ())

--------------------------------------------------------------------------------

-- | check whether to exists specified header or not. since 0.6.0.0.
hasHeader :: Monad actM => HT.HeaderName -> ApiaryT exts prms actM m a -> ApiaryT exts prms actM m a
...
header :: Monad actM => HT.HeaderName
       -> ApiaryT exts (S.ByteString ': prms) actM m a -> ApiaryT exts prms actM m a
...
header' :: (Strategy.Strategy w, Monad actM)
        ...
        -> (HT.Header -> Bool)
        -> Maybe Html
        -> ApiaryT exts (Strategy.SNext w prms S.ByteString) actM m a
        -> ApiaryT exts prms actM m a
...
accept :: Monad actM => ContentType -> ApiaryT exts prms actM m a -> ApiaryT exts prms actM m a
